...
def combined_gauge_volume(square_theta_phy_sample_1, square_theta_phy_detector_1, square_theta_phy_sample_2, square_theta_phy_detector_2,
                          square_theta_phy_sample_3, square_theta_phy_detector_3, sample_points_x_y):
    r"""
     Calculate the non-zero gauge volume for different positions of the sample.

    Parameters
    ----------
    square_theta_phy_sample_n : tuple
        the tuple of theta, phi of the four points of the collimator squared opening at
        sample side where each element of the tuple is the array of theta/phi of four points of the
        collimator for a particular point of the sample.
        e.g. (array([theta1, theta2, theta3, theta4]), array([phi1, phi2, phi3, phi4]))
        ** n represents the sections of the collimator **

     square_theta_phy_detector_n : tuple
        the tuple of theta, phi of the four points of the collimator squared opening at
        detector side where each element of the tuple is the array of theta/phi of four points of the
        collimator for a particular point of the sample.
        e.g. (array([theta1, theta2, theta3, theta4]), array([phi1, phi2, phi3, phi4]))
    ...
    theta_phiS_arr_1 = np.array(square_theta_phy_sample_1) #shape: (2, pointsNum, 4)
    theta_phiS_1 =  theta_phiS_arr_1.transpose(2, 1, 0).transpose(1, 0, 2) #shape: (pointsNum, 4,2)
    theta_phiS_list_1 = theta_phiS_1.tolist()
    theta_phiD_array_1 = np.array(square_theta_phy_detector_1).transpose(2,1,0).transpose(1,0,2)
    theta_phiD_list_1=theta_phiD_array_1.tolist()


    theta_phiS_arr_2 = np.array(square_theta_phy_sample_2) #shape: (2, pointsNum, 4)
    theta_phiS_2 =  theta_phiS_arr_2.transpose(2, 1, 0).transpose(1, 0, 2) #shape: (pointsNum, 4,2)
    theta_phiS_list_2 = theta_phiS_2.tolist()
    theta_phiD_array_2 = np.array(square_theta_phy_detector_2).transpose(2,1,0).transpose(1,0,2)
    theta_phiD_list_2=theta_phiD_array_2.tolist()

    theta_phiS_arr_3 = np.array(square_theta_phy_sample_3)  # shape: (2, pointsNum, 4)
    theta_phiS_3 = theta_phiS_arr_3.transpose(2, 1, 0).transpose(1, 0, 2)  # shape: (pointsNum, 4,2)
    theta_phiS_list_3 = theta_phiS_3.tolist()
    theta_phiD_array_3 = np.array(square_theta_phy_detector_3).transpose(2, 1, 0).transpose(1, 0, 2)
    theta_phiD_list_3 = theta_phiD_array_3.tolist()



    for i in range(theta_phiS_1.shape[0]):

        s_1=Polygon(theta_phiS_list_1[i])
        d_1 = Polygon(theta_phiD_list_1[i])

        s_2 = Polygon(theta_phiS_list_2[i])
        d_2 = Polygon(theta_phiD_list_2[i])

        s_3 = Polygon(theta_phiS_list_3[i])
        d_3 = Polygon(theta_phiD_list_3[i])

        if s_1.intersects(d_1):
            gauge_volume.append(s_1.intersection(d_1).intersection(s_2).intersection(d_2).intersection(s_3).intersection(d_3).area / d_3.area)
            sample_points_x_nonZero.append(sample_points_x_y[0, i])
            sample_points_y_nonZero.append(sample_points_x_y[1, i])

        # gauge_volume.append(s_1.intersection(d_1).intersection(s_2).intersection(d_2).intersection(s_3).intersection(
        #     d_3).area / d_3.area)
        # sample_points_x_nonZero.append(sample_points_x_y[0, i])
        # sample_points_y_nonZero.append(sample_points_x_y[1, i])
